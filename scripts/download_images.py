...
def download_image(index, url):
    filename = f"ai_mockups/mockup_{index+1}.jpg"
    try:
        req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
        with urllib.request.urlopen(req) as response, open(filename, 'wb') as out_file:
            out_file.write(response.read())
        print(f"Đã tải ảnh {index+1}/30")
        return filename
    except Exception as e:
        print(f"Lỗi tải ảnh {index+1}: {e}")
        return None

with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
    futures = [executor.submit(download_image, i, url) for i, url in enumerate(urls)]
...
# Replaces the pollinations API urls with local file paths
